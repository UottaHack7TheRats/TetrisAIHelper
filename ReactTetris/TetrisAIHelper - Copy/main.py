...
decision_tree_2ndbest = None
decision_tree_3rdbest = None
...
while not done:
    ...
    decision_tree_2ndbest = next_states_scored[1][1]
    decision_tree_3rdbest = next_states_scored[2][1]
    # Move all down
    ...
    if decision_tree_3rdbest is not None:
        recomended = moveGhostDown(decision_tree_3rdbest[1])
        for i in range(4):
            for j in range(4):
                if i * 4 + j in recomended.image():
                    pygame.draw.rect(screen, (200, 120, 128), [   
    ...
    if decision_tree_2ndbest is not None:
        recomended = moveGhostDown(decision_tree_2ndbest[1])
        for i in range(4):
            for j in range(4):
                if i * 4 + j in recomended.image():
                    pygame.draw.rect(screen, (Color.light_blue), [   
                        game.x + game.zoom * (j + recomended.x) + 1,
                        game.y + game.zoom * (i + recomended.y) + 1,
                        game.zoom - 2,
                        game.zoom - 2
